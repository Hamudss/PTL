...
def get_data_api(endpoint, params, cliente):
  ...
  headers = {'content-type': 'application/json', 'Authorization' : 'Bearer ' + get_cliente_key(cliente)}
  try:
...
def get_extenso(tipo = 'numerico', valor='0', cliente=''):
  url = 'https://api-agnes.preambulo.com.br/api/v1/extenso?tipo='+ tipo +'&valor='+ valor
  headers = {'content-type': 'application/json', 'Authorization' : 'Bearer ' + get_cliente_key(cliente)}
  try:
    r = requests.get(url, headers= headers)
  except requests.exceptions.RequestException as e:
